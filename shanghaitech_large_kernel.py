...
class MCNN:
    ...
    def data_pre_train(self, kind, dataset):  # kind = train || val
        ...
        img_names = os.listdir(img_path)  # all file names under the dir
    ...
    def data_pre_test(self, dataset):
        ...
        data = []
        for i in range(1, img_num + 1):
            if i==104: # found a nan in IGM_104.csv, need to recreate gt data
                continue
            if i % 50 == 0:
                print(i, '/', img_num)
            name = 'IMG_' + str(i) + '.jpg'
    ...
    def inf(self, x):
        ...
        w_conv5 = tf.get_variable('w_conv5', [1, 1, 8, 1])
        b_conv5 = tf.get_variable('b_conv5', [1])
        h_conv5 = self.conv2d(h_conv4_3, w_conv5) + b_conv5
    ...
    def train(self, max_epoch):
        with tf.Session() as sess:
            if not os.path.exists('./model_L_kernel'):
                sess.run(tf.global_variables_initializer())
            else:
                saver = tf.train.Saver()
                saver.restore(sess, './model_L_kernel/model.ckpt')

            data_train = self.data_pre_train('train', self.dataset)  # 训练数据预处理，density map缩小1/4
            ...
            for epoch in range(max_epoch):
                ...
                for i in range(len(data_train)):
                    ...
                    x_in = np.reshape(data[0],
                                      (1, data[0].shape[0], data[0].shape[1], 1))  # data[0] is image，data[1] is density map
                ...
                if val_mae < best_mae:
                    ...
                    saver = tf.train.Saver()
                    saver.save(sess, './model_L_kernel/model.ckpt')
                else:
