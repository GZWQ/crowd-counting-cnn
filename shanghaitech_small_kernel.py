...
class MCNN:
    ...
    def data_pre_test(self, dataset):
        ...
        for i in range(1, img_num + 1):
            ...
            img = np.array(img)
            img = (img - 127.5) / 128
            den = pd.read_csv(den_path + name[:-4] + '.csv', delimiter=",",header=None,error_bad_lines=False,keep_default_na=False).as_matrix()
    ...
    def inf(self, x):
        ...
        h_conv5 = self.conv2d(h_conv4_1, w_conv5) + b_conv5
    ...
    def train(self, max_epoch):
        with tf.Session() as sess:
            if not os.path.exists('./model_S_kernel'):
                sess.run(tf.global_variables_initializer())
            else:
                saver = tf.train.Saver()
                saver.restore(sess, './model_S_kernel/model.ckpt')
            ...
            for epoch in range(max_epoch):
                ...
                if val_mae < best_mae:
                    ...
                    saver = tf.train.Saver()
                    saver.save(sess, './model_S_kernel/model.ckpt')
                else:
